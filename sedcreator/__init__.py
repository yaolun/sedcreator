...
class FluxerContainer:
    ...
    def get_info(self):
        ...
        if "CD1_1" in header:
            ...
            raise Exception(
                "No CD nor CDELT were found in the header, so no pixscale could be retrieved"
            )
        pixel_scale_arcsec = pixel_scale_deg * 3600.0  # arcsec

        print(
            "The aperture used is",
            round(self.aper_rad_pixel * pixel_scale_arcsec, 3),
            "arcsec",
        )
    ...
    def plot(
        ...
        if "OBSERVAT" in header:
            if "SOFIA" in header["OBSERVAT"]:
                ...
                data_for_norm = data[
                    int(self.y_source - 5.0 * self.aper_rad_pixel) : int(
                        self.y_source + 5.0 * self.aper_rad_pixel
                    ),
                    int(self.x_source - 5.0 * self.aper_rad_pixel) : int(
        ...
        if ("TELESCOP" in header) & ("BAND" in header):
            ...
            if "BUNIT" in header:
                if (
                    ...
                    fluc_error = (
                        unit_factor_Jy * fluc_error / (beam / pixel_scale_deg**2)
                    )  # Jy
                ...
                if (
                    ...
                    fluc_error = (
                        unit_factor_Jy * fluc_error * pixel_scale_arcsec**2
                    )  # Jy
                else:
                    raise Exception(
                        "FUNITS (",
...
class FitterContainer:
    ...
    def get_model_info(
        self, keys=["mcore", "sigma", "mstar", "theta_view"], tablename=None
    ):
        """
...
class SedFitter(object):
    ...
    def chisq(self, flux_model, av):
        ...
        if treat_errors == "log":
            ...
            nfit_nonlimit = len(
                errup_fit_lin_arr[errup_fit_lin_arr < 1.0e30]
            )  # updating the number of points considered to be upper limits
            chisq_nonlimit = chisq * float(nfit) / float(nfit_nonlimit)
            ...
            for model_data, model_idx in tqdm(
                ...
                if treat_errors == "log":
                    ...
                    for av in AV_array:
                        ...
                        errup_fit_lin_arr = (
                            linear_err  # this is absolute error in linear space
                        )
                        errlo_fit_lin_arr = (
                            linear_err  # this is absolute error in linear space
                        )
    ...
    def get_average_model(
        ...
        if chisq_cut is not None and core_radius_cut is None:
            ...
            average_model_table_chisq_rcore = models[
                (models["chisq"] <= chisq_cut) & (models["rcore"] <= core_radius_cut_pc)
            ]
...
class ModelPlotter(FitterContainer):
    ...
    def plot_multiple_seds(
        ...
        cmap = plt.cm.ScalarMappable(
            cmap=cmap,
            norm=colors.LogNorm(vmin=models["chisq"].min(), vmax=models["chisq"].max()),
        )
        # cmap.set_array([])
        ...
        for ind_mod in models:
            ...
            plt.plot(
                ...
                "-",
                linewidth=0.5,
                zorder=-ind_mod["chisq"],
                c=cmap.to_rgba(ind_mod["chisq"]),
            )
        ...
        plt.plot(
            ...
            "k-",
            linewidth=1.0,
            zorder=-best_model["chisq"].data[0],
        )
        source_nu_Fnu = (
            c_micron_s / self.lambda_array * self.flux_array * Jy2erg_s_cm2
        )  # erg s-1 cm-2
        ...
        if colorbar:
            ...
            cbar.set_ticks(
                np.logspace(
                    np.log10(models["chisq"].min()),
                    np.log10(models["chisq"].max()),
                    num=5,
                )
            )
            # trick to consider the adecuate number of decimals in round
            dec = (
                int(np.floor(np.abs(np.log10(models["chisq"].min())))) + 1
            )  # it has to be an integer
            cbar.set_ticklabels(
                np.around(
                    np.logspace(
    ...
    def plot2d(
        ...
        cbar.set_ticks(
            np.logspace(
                np.log10(triple_sigma_ms["chisq"].min()),
                np.log10(triple_sigma_ms["chisq"].max()),
                num=5,
            )
        )
        # trick to consider the adecuate number of decimals in round
        dec = (
            int(np.floor(np.abs(np.log10(triple_sigma_ms["chisq"].min())))) + 1
        )  # it has to be an integer
        cbar.set_ticklabels(
            np.around(
                np.logspace(
